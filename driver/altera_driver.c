/*
              DRIVER DO PROJETO DE DRIVER INTERFACE HARDWARE-SOFTWARE
...
static void *firstDisplays; //nome do PIO dos ultimos displays
static void *greenLeds; //nome do PIO dos leds verdes;
static void *redLeds; //nome do PIO dos leds vermelhos;
static void *buttons; //nome do PIO dos botões;
static void *lcd;
...
static ssize_t char_device_read(struct file *filep, char *buf, size_t len, loff_t *off) {
  ...
  if (ptr->qsys == 1)
  {
      ...
      put_user(switches & 0x3FFFF, &(ptr->dado));

  } else if (ptr->qsys == 2)
  {
      botoes = ioread32(buttons);
      put_user(botoes & 0xF, &(ptr->dado));
  }
  return count;
}
...
static int pci_probe(struct pci_dev *dev, const struct pci_device_id *id) {
  ...
  buttons = ioremap_nocache(resource + 0xC040, 0x20);
  redLeds = ioremap_nocache(resource + 0xC080, 0x20);
  greenLeds  = ioremap_nocache(resource + 0xC0A0, 0x20);
  firstDisplays = ioremap_nocache(resource + 0xC0C0, 0x20);
  lcd = ioremap_nocache(resource + 0xC0E0, 0x20);
